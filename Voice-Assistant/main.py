...
def digital_assistant(data):
    ...
    if "which is" in data:
        ...
        def query(text, model_id="tiiuae/falcon-7b-instruct"):
            ...
            headers = {"Authorization": f"Bearer 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"}
            payload = {"inputs": text}
    ...
    if "where is" in data or "navigate to" in data:
        ...
        respond("Hold on Hriday, I will show you where " + loc + " is.")
        browser = webdriver.Chrome()
        browser.get(location_url)
        time.sleep(1)
        #search = browser.find_element_by_xpath("/html/body/jsl/div[3]/div[9]/div[3]/div[1]/div[1]/div[1]/div[2]/div[1]/button")
...
if __name__ == "__main__":
    pyttsx3.speak("Hello Hriday. what can i do for you?")
    # time.sleep(1)
    listening = True
    while listening:
        task = listen()
